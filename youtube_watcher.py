...
def fetch_playlist_items(google_api_key, youtube_playlist_id, page_token=None):
    ...
    if "items" not in payload:
        return  # Prevent crash if no items exist
    ...
    if next_page_token:
        if next_page_token == page_token:  # Prevent infinite recursion
            return
...
def fetch_videos(google_api_key, video_id, page_token=None):
    ...
    if "items" not in payload:
        return  # Prevent crashing on missing data
    ...
    if next_page_token:
        if next_page_token == page_token:  # Prevent infinite loop
            return
